int plot_dodt_nevents()
{
  ...
  TFile *fIn[2];
  fIn[0] = new TFile(Form("../data/D-photo-500000-%.0f/D_solid_photo_%.1fGeV.root",beamE,beamE),"READ");
  fIn[1] = new TFile(Form("../data/D-electro-10000000-%.0f/D_solid_electro_%.1fGeV.root",beamE,beamE),"READ");
  TTree *tIn[2];
  ...
  for(int m = 0 ; m < 2 ; m++)
      ...
      for(int i = 1; i <= h[m]->GetNbinsX() ; i++)
	{
      ...
      if(m==0) // photo
	latex.DrawLatexNDC(0.18,0.86,Form("#bf{SoLID Simulation} J/#psi #bf{#color[4]{photoproduction}} , %.1f GeV Beam",beamE));
      else if(m==1) // electro
	latex.DrawLatexNDC(0.18,0.86,Form("#bf{SoLID Simulation} J/#psi #bf{#color[2]{electroproduction}} , %.1f GeV Beam",beamE));
      ...
      hEvents[m]->GetYaxis()->SetRangeUser(.1,900000);
      ...
      legend[m]->Draw("same");
      if(m==0) // photo
	latex.DrawLatexNDC(0.18,0.85,Form("#bf{SoLID Simulation} J/#psi #bf{#color[4]{photoproduction}} , %.1f GeV Beam",beamE));
      else if(m==1) // electro
	latex.DrawLatexNDC(0.18,0.85,Form("#bf{SoLID Simulation} J/#psi #bf{#color[2]{electroproduction}} , %.1f GeV Beam",beamE));
      latex.DrawLatexNDC(0.3,0.805,"50 days at L=1.2e37 cm^{-2}s^{-1}");
      latex.DrawLatexNDC(0.3,0.765,Form("%.2f < E_{#gamma} < %.2f",Emin,Emax));
    }
  return 0;
}
